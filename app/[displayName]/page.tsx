...
export default function UserPublicPage({ params }: { params: Promise<{ displayName: string }> }) {
  const unwrappedParams = use(params);
  const displayName = decodeURIComponent(unwrappedParams.displayName);

  // 1. displayName으로 userId 찾기
  const { data: userId, isLoading: isUserIdLoading, isError: isUserIdError } = useQuery({
